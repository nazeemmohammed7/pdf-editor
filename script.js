...
const makeDraggable = (el, onDrop) => {
  ...
  window.addEventListener('mousemove', (e) => {
    ...
    const x = Math.max(0, Math.min(e.clientX - rect.left - offsetX, rect.width - 20));
...
fileInput.addEventListener('change', async (e) => {
  ...
  state.pdfjsDoc = await pdfjsLib.getDocument({ data: state.pdfBytes }).promise;
...
eraseWordBtn.addEventListener('click', () => {
  ...
  eraseWordBtn.textContent = `Erase Mode: ${state.eraseMode ? 'On' : 'Off'}`;
});
...
overlay.addEventListener('click', (e) => {
  ...
  getPageEdits(state.currentPage).erases.push({ x: x - 20, y: y - 12, w: 70, h: 24 });
  renderOverlay();
});
